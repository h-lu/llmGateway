...
async def update_student_quota(
    session: AsyncSession, student_id: str, tokens_used: int, auto_commit: bool = True
) -> tuple[bool, int, int]:
...
async def check_and_consume_quota(
    session: AsyncSession, student_id: str, tokens_needed: int, auto_commit: bool = True
) -> tuple[bool, int, int]:
    ...
    if row is None:
        ...
        result = await session.execute(
            select(Student.used_quota, Student.current_week_quota).where(
                Student.id == student_id
            )
        )
        row = result.fetchone()
