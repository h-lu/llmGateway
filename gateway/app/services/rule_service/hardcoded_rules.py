...
async def evaluate_prompt_async(
    ...
    for pattern_str, message in GUIDE_PATTERNS:
        try:
            ...
            if match:
                return RuleResult(
                    action="guided", message=message, rule_id=f"hardcoded:{pattern_str}"
                )
        except re.error as e:
...
def evaluate_prompt(
    ...
    try:
        ...
        logger.warning(
            "evaluate_prompt called from async context, consider using evaluate_prompt_async"
        )
        # Fallback: schedule in the running loop and wait (may block)
        with concurrent.futures.ThreadPoolExecutor() as pool:
            future = pool.submit(
                asyncio.run, evaluate_prompt_async(prompt, current_week, is_conceptual)
            )
