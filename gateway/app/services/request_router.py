...
class RequestRouter:
    ...
    async def acquire_streaming_slot(self) -> bool:
        ...
        try:
            await asyncio.wait_for(
                self._streaming_semaphore.acquire(), timeout=self.timeout
            )
    ...
    async def acquire_normal_slot(self) -> bool:
        ...
        try:
            await asyncio.wait_for(
                self._normal_semaphore.acquire(), timeout=self.timeout
            )
            async with self._lock:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "streaming": {
                ...
                "utilization": round(self._streaming_active / self.streaming_limit, 4)
            ...
            "normal": {
                ...
                "utilization": round(self._normal_active / self.normal_limit, 4)
            ...
            "capacity": {
                ...
                "total_utilization": round(
                    ...
                    / (self.streaming_limit + self.normal_limit),
                    4,
                )
                if (self.streaming_limit + self.normal_limit) > 0
                else 0,
            },
        }
