...
class MetricsCollector:
    ...
    _requests: Dict[str, RequestMetrics] = field(default_factory=lambda: defaultdict(lambda: RequestMetrics()))
        ...
        async with self._lock:
            ...
            return {
                ...
                "providers": {
                    ...
                    for name in set(list(self._provider_health.keys()) + list(self._provider_requests.keys()))
                },
                "quota": {
                    "checks": self._quota_checks,
                    "exceeded": self._quota_exceeded,
                    "exceeded_rate": round(self._quota_exceeded / self._quota_checks, 4) if self._quota_checks > 0 else 0
                },
    ...
    async def get_prometheus_metrics(self) -> str:
        ...
        async with self._lock:
            ...
            for endpoint, metrics in self._requests.items():
                lines.append(f'gateway_requests_total{{endpoint="{endpoint}"}} {metrics.count}')
            ...
            for endpoint, metrics in self._requests.items():
                lines.append(f'gateway_request_duration_seconds{{endpoint="{endpoint}"}} {metrics.total_duration}')
            ...
            for provider, healthy in self._provider_health.items():
                ...
                lines.append(f'gateway_provider_health{{provider="{provider}"}} {health_value}')
            ...
            for provider, count in self._provider_requests.items():
                lines.append(f'gateway_provider_requests_total{{provider="{provider}"}} {count}')
