...
class ProviderFactory:
    ...
    def create_provider(
        self, provider_type: ProviderType, config: Optional[ProviderConfig] = None
    ) -> BaseProvider:
        ...
        if config is None:
            if provider_type not in self._configs:
                raise RuntimeError(
                    f"No configuration available for provider: {provider_type}"
                )
    ...
    def create_primary_provider(self) -> BaseProvider:
        ...
        enabled_configs = [
            (ptype, config) for ptype, config in self._configs.items() if config.enabled
        ]
...
def get_load_balancer(
    http_client: Optional[httpx.AsyncClient] = None, strategy: str = "round_robin"
) -> LoadBalancer:
    ...
    if _load_balancer is None:
        ...
        _load_balancer = LoadBalancer(
            factory=factory, health_checker=health_checker, strategy=strategy
        )
